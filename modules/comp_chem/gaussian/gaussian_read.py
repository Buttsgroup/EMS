...
def gauss_nmr_read(file, float_dtype=np.float64, int_dtype=np.int32):
    """
    ...
    shift_array = np.zeros(atomnumber, dtype=float_dtype)
    couplings = np.zeros((atomnumber, atomnumber), dtype=float_dtype)
    atom_types = np.zeros(atomnumber, dtype=int_dtype)
    atom_coords = np.zeros((atomnumber, 3), dtype=float_dtype)
        ...
        for line in f_handle:
            ...
            if coupling_switch:
                ...
                if "D" not in line:
                    ...
                    items = line.split()
                    try:
                        i_indices = np.asarray(items, dtype=int_dtype)
                    except:
